...
import type { PublicShopResponseDto } from '@/modules/shop-profile/presentation/dto/public-shop-response.dto';
import type { ListPublicShopsQueryDto } from '@/modules/shop-profile/presentation/dto/list-public-shops-query.dto';
import type { PublicShopListResponseDto } from '@/modules/shop-profile/presentation/dto/public-shop-list-response.dto';
...
export class PublicShopService {
    ...
    async follow(
        ...
        if (!viewerId)
            throw new UnauthorizedException(
                'Vui lòng đăng nhập để theo dõi shop.',
            );
        const shop = await this.findShop(identifier);
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            if (lockedShop.status !== ShopStatus.ACTIVE) {
                throw new ConflictException(
                    'Shop hiện không nhận người theo dõi mới.',
                );
            }
    ...
    async unfollow(
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            lockedShop.followerCount = Math.max(
                0,
                lockedShop.followerCount - 1,
            );
            const ownerShop = ownerShopSnapshot?.id
                ...
                : undefined;
            if (ownerShop && ownerShop.id !== lockedShop.id)
                ownerShop.followingCount = Math.max(
                    0,
                    ownerShop.followingCount - 1,
                );
            await manager
