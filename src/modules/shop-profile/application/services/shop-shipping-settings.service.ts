...
} from '@/modules/shop-profile/presentation/dto/shop-shipping.dto';
...
export class ShopShippingSettingsService {
    ...
    async updateSettings(
        ...
        Object.assign(settings, {
            ...
            pickupWindowStart:
                dto.pickupWindowStart ?? settings.pickupWindowStart,
    ...
    async createAddress(
        ...
        const configuredDefault = settings.defaultPickupAddressId
            ? await this.addressRepository.findOne({
                  where: {
                      id: settings.defaultPickupAddressId,
                      shopId: shop.id,
                  },
              })
            : null;
    ...
    async deleteAddress(
        ...
        await this.addressRepository.manager.transaction(async (manager) => {
            ...
            const settingsRepository =
                manager.getRepository(ShopShippingSettings);
    ...
    async getDefaultForShop(shopId: string): Promise<ShopPickupAddress> {
        ...
        const addresses = await this.ensureOnboardingPickupAddress(
            shop,
            settings,
        );
